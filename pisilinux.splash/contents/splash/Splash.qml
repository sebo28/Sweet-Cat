...
Rectangle {
    ...
    gradient: Gradient {
        GradientStop { position: 0.0; image: "images/çubuk.png" }
        GradientStop { position: 1.0; image: "images/çubuk.png" }
    }
    //Get KDE splash screen stages
    onStageChanged: {
        if (stage==1) {
            beltOuter.opacity=0.3
        }
        if (stage==2) {
        }
        if (stage==3) {
            beltInner.opacity=0.3
        }
        if (stage==4) {
        }
        if (stage==5) {
    ...
    Image {
        id: imageConfig
        property var imageArray: ["images/render.png", "images/snowmiku2018.png"]
        // Percentage of Screen configs
        property int maxHeight: 80
        property int maxWidth: 70
        property int x_offset: 5
        opacity: 0
    ...
    Rectangle {
        ...
        Behavior on secondAnimation {
            NumberAnimation {target: beltInner; property: "width"; to: root.width * 0.62; duration: 1000 }
        }
    }
